...
class HyperbolicAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        manifold = graphzoo.manifolds.Hyperboloid()
        self.encoder = nn.Sequential(
            graphzoo.layers.HyperbolicGraphConvolution(manifold, args.dim, args.hidden_dim, 1, 1, 0, nn.ReLU(), True,
                                                       True, True),
            graphzoo.layers.HyperbolicGraphConvolution(manifold, args.hidden_dim, args.hidden_dim, 1, 1, 0, nn.ReLU(), True,
                                                       True, True),
            graphzoo.layers.HyperbolicGraphConvolution(manifold, args.hidden_dim, args.hidden_dim, 1, 1, 0, nn.ReLU(), True,
                                                       True, True),
            graphzoo.layers.HyperbolicGraphConvolution(manifold, args.hidden_dim, args.dim, 1, 1, 0, nn.ReLU(), True,
                                                       True, True),
        )
    ...
    def forward(self, x, h, node_mask, edge_mask):
        ...
        edges = self.get_adj_matrix(n_nodes,
                                    batch_size)  # [rows, cols] rows=cols=(batch_size*n_nodes*n_nodes) value in [0,batch_size*n_nodes)
