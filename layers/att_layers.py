...
class DenseAtt(nn.Module):
    ...
    def forward(self, x, edge_attr,edge_index,edge_mask):
        row, col = edge_index
        b_n = x.size(0)
        if self.edge_dim > 0:
            x_cat = torch.concat((x[row], x[col], edge_attr), dim=1)  # (b*n*n,2*dim+1)
        else:
            x_cat = torch.concat((x[row], x[col]), dim=1)  # (b*n*n,2*dim+1)
        att = self.att_mlp(x_cat)  # (b*n_node*n_node,1)
        pad_mask = 1-edge_mask
        zero_vec = -9e15 * pad_mask
        att = att+zero_vec  # padding的地方会-9e15
        att = nn.Softmax(dim=-1)(att.view(b_n,-1))
        att = F.dropout(att, self.dropout, training=self.training).view(-1,1)
