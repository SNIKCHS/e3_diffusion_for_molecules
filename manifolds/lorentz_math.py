...
def dist(x, y, *, k, keepdim=False, dim=-1, expand_k=False):
    r"""
    ...
    """
    return _dist(x, y, k=k, keepdim=keepdim, dim=dim, expand_k=expand_k)


@torch.jit.script
def _dist(x, y, k: torch.Tensor, keepdim: bool = False, dim: int = -1, expand_k: bool = False):
    if expand_k:
        b_n_nodes = k.size(0)
...
def project(x, *, k, dim=-1, k_is_vary=False):
    r"""
    ...
    """
    return _project(x, k=k, dim=dim, k_is_vary=k_is_vary)


@torch.jit.script
def _project(x, k: torch.Tensor, dim: int = -1, k_is_vary: bool = False):
    dn = x.size(dim) - 1
    if k_is_vary:
        left_ = torch.sqrt(
            k + torch.norm(x.narrow(dim, 1, dn), p=2, dim=dim, keepdim=True) ** 2
        )
    else:
        left_ = torch.sqrt(
...
def logmap(x, y, *, k, dim=-1,expand_k=False):
    r"""
    ...
    """
    return _logmap(x, y, k=k, dim=dim,expand_k=expand_k)


@torch.jit.script
def _logmap(x, y, k, dim: int = -1,expand_k:bool=False):
    if expand_k:
        b_n_nodes = k.size(0)
        n_nodes = x.size(0)//b_n_nodes
        b = b_n_nodes//n_nodes
